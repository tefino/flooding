...
void LocalRV::requestTMAssistanceForRendezvous(InformationItem *pub, RemoteHostSet &_publishers, RemoteHostSet &_subscribers, IdsHashMap &IDs) {
    ...
    packet_len = /*For the blackadder API*/ sizeof (typeForAPI) + sizeof (IDLenForAPI) + 2 * PURSUIT_ID_LEN + sizeof (strategy) + FID_LEN/*END OF API*/\
            /*PAYLOAD*/ + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) /*sizeof(numberOfPubs)*/ + \
            _publishers.size() * NODEID_LEN + sizeof (no_subscribers) /*sizeof(numberOfSubs)*/ + _subscribers.size() * NODEID_LEN \
                    + sizeof (no_ids) /*sizeof(numberOFIDs)*/ + pub->ids.size() * sizeof (unsigned char)/*# of fragments for each ID*/ +IDs_total_bytes;
    ...
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScope.length()\
    		+ sizeof (strategy) + FID_LEN + sizeof (request_type), &pub->strategy, sizeof (pub->strategy));
    /*put the publisher IDs*/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScope.length() \
    		+ sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy), &no_publishers, sizeof (no_publishers));
    for (RemoteHostSetIter iter = _publishers.begin(); iter != _publishers.end(); iter++) {
        memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScope.length() \
			+ sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) + publisher_index, (*iter)._rhpointer->remoteHostID.c_str(), (*iter)._rhpointer->remoteHostID.length());
...
void LocalRV::notifyLocalSubscriber(unsigned char type, StringSet &IDs) {
    ...
    /**********************************************/
    p = Packet::make(30, NULL, /*For the blackadder API*/ sizeof (typeForAPI) + sizeof (IDLenForAPI) + \
    gc->notificationIID.length() + sizeof (strategyAPI) + FID_LEN/*END OF API*/ + sizeof (type) + \
    sizeof (numberOfIDs)+((int) numberOfIDs) * sizeof (unsigned char) +totalIDsLength, 0);
    memcpy(p->data(), &typeForAPI, sizeof (typeForAPI));
    memcpy(p->data() + sizeof (typeForAPI), &IDLenForAPI, sizeof (IDLenForAPI));
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI), gc->notificationIID.c_str(), gc->notificationIID.length());
    ...
    /***********************************************/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->notificationIID.length() + sizeof (strategyAPI) + FID_LEN,\
    &type, sizeof (type));
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->notificationIID.length() + sizeof (strategyAPI) + FID_LEN + sizeof (type),\
